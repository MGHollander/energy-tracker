...
export async function exportAllReadingsOverwrite(houseId?: string) {
  ...
  const header = 'id,date,electricity_high,electricity_low,gas,water,user_id,house_id,created_at,updated_at\n'
  const rows = readings.map(reading =>
    `${reading.id},${reading.date},${reading.electricity_high},${reading.electricity_low},${reading.gas},${reading.water},${reading.user_id},${reading.house_id},${reading.created_at},${reading.updated_at}\n`
  ).join('')
